...
inline void render_route(sptr::ShipNavRoute route) {
  if (ImGui::CollapsingHeader("Route")) {
    ...
    ImGui::LabelText("Departure Time", "%s",
                     route->getDepartureTime().to_string().c_str());
...
struct ShipDetailsLayer : public Layer {
    ...
    if (ship) {
      ...
      if (ImGui::BeginTabBar("tabs", tab_bar_flags)) {
        if (ImGui::BeginTabItem("Status")) {
          ...
          ImGui::Text("Location");
          ImGui::SameLine();
          auto wp = sptr::get_waypoint(ship);
          if (wp) {
            waypoint = wp;
          }
          if (waypoint) {
          ...
          ImGui::Text(
              "%s",
              ship->getNav()->getFlightMode()->toJson().as_string().c_str());

          ImGui::Text("Nav Status");
          ImGui::SameLine();
          ImGui::Text(
              "%s", ship->getNav()->getStatus()->toJson().as_string().c_str());
        ...
        if (ImGui::BeginTabItem("Waypoints")) {
          ...
          for (sptr::Waypoint wp : gs.waypoints) {
            render_waypoint(wp);
          }
          ImGui::EndTabItem();
        }
  ...
  void render_traits_combo() {
    ...
    sptr::GameState &gs = sptr::GameState::instance();
    if (ImGui::Combo("Traits", &gs.waypointsCombo, TRAITS)) {
      auto trait = std::make_shared<models::WaypointTraitSymbol>();
      trait->setValue(
          static_cast<models::WaypointTraitSymbol::eWaypointTraitSymbol>(
              gs.waypointsCombo));
      sptr::DataProvider::instance().request_waypoints(
          ship->getNav()->getSystemSymbol().c_str(), trait);
    }
  }
