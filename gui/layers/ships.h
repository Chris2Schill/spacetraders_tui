...
inline void render(const sptr::ShipEngine& engine) {
    std::string header =
        fmt::format("Engine - {} (s{},c{})", engine->getName(),
...
inline void render(const sptr::ShipFrame& frame) {
    std::string header = fmt::format("Frame - {} (c{})", frame->getName(),
                                     frame->getCondition());
...
inline void render(const sptr::ShipFuel& fuel) {
    ...
    std::string overlay =
        fmt::format("{}/{}", fuel->getCurrent(), fuel->getCapacity());
...
inline void render(const Ref<models::Cooldown> cd) {
}

inline void render_ship(const sptr::Ship& ship) {
    if (ImGui::CollapsingHeader(ship->getSymbol().c_str())) {
        ...
        if (ImGui::Button("View")) {
            ...
            sptr::GameState::instance().set_selected_ship(ship);
        }
        ImGui::PopID();
    }
}
...
inline void render_ships() {
    const sptr::GameState& gs = sptr::GameState::instance();
    auto view = gs.scene->view<sptr::ShipComponent>();
    ...
    if (ImGui::BeginTabBar("tabs", tab_bar_flags)) {
        if (ImGui::BeginTabItem("All")) {
            view.each([](const auto& shipc) { render_ship(shipc.ship); });
        ...
        if (ImGui::BeginTabItem("Explorers")) {
            ...
            view.each([](const auto& shipc) {
                if (shipc.ship->getRegistration()->getRole()->getValue() ==
                    models::ShipRole::eShipRole::ShipRole_EXPLORER) {

                    render_ship(shipc.ship);
                }
            });
            ImGui::EndTabItem();
        }
