...
ScannedShip_frame::ScannedShip_frame()
{
...
void ScannedShip_frame::validate()
{
    // TODO: implement validation
}

web::json::value ScannedShip_frame::toJson() const
{
...
bool ScannedShip_frame::fromJson(const web::json::value& val)
{
    bool ok = true;
...
void ScannedShip_frame::toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix) const
{
...
bool ScannedShip_frame::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    bool ok = true;
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t(U(".")))
...
utility::string_t ScannedShip_frame::getSymbol() const
{
    return m_Symbol;
}

void ScannedShip_frame::setSymbol(const utility::string_t& value)
{
    m_Symbol = value;
    m_SymbolIsSet = true;
}

bool ScannedShip_frame::symbolIsSet() const
{
    return m_SymbolIsSet;
}

void ScannedShip_frame::unsetSymbol()
{
    m_SymbolIsSet = false;
}
}
