...
ScannedShip_engine::ScannedShip_engine()
{
...
void ScannedShip_engine::validate()
{
    // TODO: implement validation
}

web::json::value ScannedShip_engine::toJson() const
{
...
bool ScannedShip_engine::fromJson(const web::json::value& val)
{
    bool ok = true;
...
void ScannedShip_engine::toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix) const
{
...
bool ScannedShip_engine::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    bool ok = true;
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t(U(".")))
...
utility::string_t ScannedShip_engine::getSymbol() const
{
    return m_Symbol;
}

void ScannedShip_engine::setSymbol(const utility::string_t& value)
{
    m_Symbol = value;
    m_SymbolIsSet = true;
}

bool ScannedShip_engine::symbolIsSet() const
{
    return m_SymbolIsSet;
}

void ScannedShip_engine::unsetSymbol()
{
    m_SymbolIsSet = false;
}
}
