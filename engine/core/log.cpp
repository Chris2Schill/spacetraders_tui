...
void AssertLog(const char* str, const char* file, int line, bool cond) {
   if (!cond)  {
        LOG_ERROR("Assert failed: {}:{} - {}", file, line, str);
        // ::Log::GetClientLogger()->error("Assert failed: {}:{} - {}", file, line, str);
   }
}
