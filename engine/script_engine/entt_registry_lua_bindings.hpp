...
template <typename Component>
auto addComponent(Entity* entity, const sol::table& instance, sol::this_state s) {
  LOG_CORE_TRACE("addComponent");
  auto& comp = entity->addComponent<Component>(instance.valid() ? instance.as<Component>() : Component{});
  return sol::make_reference(s, std::ref(comp));
}
template <typename Component>
auto hasComponent(Entity* entity) {
  LOG_CORE_TRACE("hasComponent");
...
template <typename Component> void register_meta_component() {
  ...
  entt::meta<Component>()
    ...
    .template func<&getComponent<Component>>("getComponent"_hs)
    .template func<&addComponent<Component>>("addComponent"_hs)
    .template func<&hasComponent<Component>>("hasComponent"_hs);
}
